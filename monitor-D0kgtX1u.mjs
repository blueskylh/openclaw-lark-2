...
import { C as getChatTypeFeishu, D as ticketElapsed, E as getTicket, M as getLarkAccount, N as getLarkAccountIds, O as withTicket, S as getUserAgent, T as larkLogger, _ as extractPermissionGrantUrl, b as getResolvedConfig, c as AppScopeCheckFailedError, d as MESSAGE_TERMINAL_CODES, f as NeedAuthorizationError, g as UserScopeInsufficientError, h as UserAuthRequiredError, i as resolveUserName, j as getEnabledLarkAccounts, k as createAccountScopedConfig, l as AppScopeMissingError, m as TOKEN_RETRY_CODES, n as createBatchResolveNames, p as REFRESH_TOKEN_RETRYABLE, r as getUserNameCache, s as permissionErrorNotifiedAt, t as batchResolveUserNames, u as LARK_ERROR, v as extractPermissionScopes, w as isThreadCapableGroup, x as getPluginVersion, y as LarkClient } from "./user-name-cache-C_AK4ymA.mjs";
...
*/
const log$23 = larkLogger("outbound/media");
/**
...
async function sendImageLark(params) {
	...
	log$23.info(`sendImageLark: target=${to}, imageKey=${imageKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
...
async function sendFileLark(params) {
	...
	log$23.info(`sendFileLark: target=${to}, fileKey=${fileKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	return sendMediaMessage({
		client,
		to,
...
async function sendVideoLark(params) {
	const { cfg, to, fileKey, replyToMessageId, replyInThread, accountId } = params;
	log$23.info(`sendVideoLark: target=${to}, fileKey=${fileKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	return sendMediaMessage({
		client,
		to,
...
async function sendAudioLark(params) {
	const { cfg, to, fileKey, replyToMessageId, replyInThread, accountId } = params;
	log$23.info(`sendAudioLark: target=${to}, fileKey=${fileKey}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	return sendMediaMessage({
		client,
		to,
...
async function uploadAndSendMediaLark(params) {
	...
	log$23.info(`uploadAndSendMediaLark: target=${to}, source=${mediaBuffer ? "buffer" : mediaUrl ?? "(none)"}, fileName=${fileName ?? "(auto)"}`);
	let buffer;
	let resolvedFileName = fileName ?? "file";
	if (mediaBuffer) {
		buffer = mediaBuffer;
		log$23.debug(`using provided buffer: ${buffer.length} bytes`);
	} else if (mediaUrl) {
		buffer = await fetchMediaBuffer(mediaUrl, mediaLocalRoots);
		log$23.debug(`fetched media: ${buffer.length} bytes from "${mediaUrl}"`);
		if (!fileName) {
	...
	log$23.info(`resolved: fileName="${resolvedFileName}", type=${isImage ? "image" : "file"}, size=${buffer.length}`);
	if (isImage) {
		...
		log$23.debug(`image uploaded: imageKey=${imageKey}`);
	...
	log$23.debug(`file uploaded: fileKey=${fileKey}, fileType=${fileType}${isAudio || isVideo ? `, duration=${duration ?? "unknown"}ms` : ""}`);
...
async function validateRemoteUrl(raw) {
	...
	if (net.isIP(hostname)) {
		...
		log$23.warn(`[feishu-media] DNS resolution failed for "${hostname}": ${err}`);
	}
}
/**
...
async function fetchMediaBuffer(urlOrPath, localRoots) {
	...
	if (isLocalMediaPath(raw)) {
		...
		log$23.debug(`local file read: "${filePath}", ${buf.length} bytes`);
		return buf;
	}
	await validateRemoteUrl(raw);
	const FETCH_TIMEOUT_MS = 3e4;
	log$23.info(`fetching remote media: ${raw}`);
	...
	log$23.debug(`remote media fetched: ${raw}, ${arrayBuffer.byteLength} bytes`);
...
const log$22 = larkLogger("outbound/deliver");
/**
...
async function sendImMessage(params) {
	const { client, to, content, msgType, replyToMessageId, replyInThread } = params;
	if (replyToMessageId) {
		log$22.info(`replying to message ${replyToMessageId} (msg_type=${msgType}, thread=${replyInThread ?? false})`);
		const response = await client.im.message.reply({
			path: { message_id: replyToMessageId },
			data: {
				content,
	...
	log$22.debug(`message created: messageId=${result.messageId}`);
	return result;
}
/**
...
async function sendTextLark(params) {
	...
	if (card) {
		...
		log$22.info(`detected ${version} card JSON in text (target=${to}), routing to sendCardLark`);
		return sendCardLark({
			cfg,
			to,
			card,
	...
	log$22.info(`sendTextLark: target=${to}, textLength=${text.length}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
...
async function sendCardLark(params) {
	...
	const version = card.schema === "2.0" ? "v2" : "v1";
	log$22.info(`sendCardLark: target=${to}, cardVersion=${version}`);
	const client = LarkClient.fromCfg(cfg, accountId).sdk;
	...
	try {
		...
		log$22.error(`sendCardLark failed: ${detail}`);
...
async function sendMediaLark(params) {
	...
	log$22.info(`sendMediaLark: target=${to}, mediaUrl=${mediaUrl}`);
	try {
		...
		log$22.info(`media sent: messageId=${result.messageId}`);
		return {
		...
		log$22.error(`sendMediaLark failed for "${mediaUrl}": ${errMsg}`);
		log$22.info(`falling back to text link for "${mediaUrl}"`);
		return {
			...await sendTextLark({
				cfg,
				to,
...
const log$21 = larkLogger("core/app-scope-checker");
...
async function getAppGrantedScopes(sdk, appId, tokenType) {
	...
	try {
		...
		log$21.info(`fetched ${validScopes.length} scopes for app ${appId}`);
		const scopes = validScopes.filter((s) => {
			if (tokenType && s.token_types && Array.isArray(s.token_types)) return s.token_types.includes(tokenType);
			return true;
		}).map((s) => s.scope);
		log$21.info(`returning ${scopes.length} scopes${tokenType ? ` for ${tokenType} token` : ""}`);
		return scopes;
	} catch (err) {
		...
		log$21.warn(`failed to fetch scopes for ${appId}: ${err instanceof Error ? err.message : err}`);
		return [];
	}
}
/**
...
const log$20 = larkLogger("core/app-owner-fallback");
/**
...
async function getAppOwnerFallback(account, sdk) {
	...
	try {
		...
		log$20.warn(`failed to get owner for ${appId}: ${err instanceof Error ? err.message : err}`);
		return;
	}
}
//#endregion
...
const log$19 = larkLogger("converters/merge-forward");
/**
...
async function expand(accountId, messageId, resolveUserName, batchResolveNames, fetchSubMessages) {
	...
	try {
		...
		log$19.error("fetch sub-messages failed", {
			messageId,
	...
	if (senderIds.length > 0 && batchResolveNames) try {
		...
		log$19.debug("batchResolveNames failed (best-effort)", { error: err instanceof Error ? err.message : String(err) });
	}
...
async function formatSubTree(parentId, childrenMap, accountId, resolveUserName) {
	...
	for (const item of children) try {
		...
		log$19.warn("failed to convert sub-message", {
...
const log$18 = larkLogger("inbound/parse-io");
/**
...
async function fetchCardContent(messageId, larkClient) {
	try {
		...
		log$18.warn(`fetchCardContent failed for ${messageId}: ${err instanceof Error ? err.message : String(err)}`);
		return;
	}
}
/**
...
function createParseResolveNames(account) {
	return createBatchResolveNames(account, (...args) => log$18.info(args.map(String).join(" ")));
}
//#endregion
//#region src/messaging/inbound/parse.ts
const log$17 = larkLogger("inbound/parse");
/**
...
async function parseMessageEvent(event, botOpenId, expandCtx) {
	...
	if (event.message.message_type === "interactive" && expandCtx) {
		...
		if (fullContent) {
			...
			log$17.info("replaced interactive content with full v2 card data");
		}
	}
	const convertCtx = {
		mentions: mentionMap,
...
const log$16 = larkLogger("shared/message-lookup");
/**
...
async function getMessageFeishu(params) {
	...
	try {
		...
		if (!items || items.length === 0) {
			log$16.info(`getMessageFeishu: no items returned for ${messageId}`);
			return null;
		}
		const expandCtx = expandForward ? {
			...
			batchResolveNames: createBatchResolveNames(getLarkAccount(cfg, accountId), (...args) => log$16.info(args.map(String).join(" ")))
		} : void 0;
		return await parseMessageItem(items[0], messageId, expandCtx);
	} catch (error) {
		log$16.error(`get message failed (${messageId}): ${error instanceof Error ? error.message : String(error)}`);
		return null;
	}
}
/**
...
const log$15 = larkLogger("outbound/typing");
/**
...
async function addTypingIndicator(params) {
	...
	try {
		...
		if (isMessageUnavailableError(error)) {
			log$15.debug(`Skip add typing indicator for unavailable message`, { messageId: normalizedId });
			return state;
		}
		log$15.debug(`Failed to add typing indicator`, {
			messageId,
...
async function removeTypingIndicator(params) {
	...
	try {
		...
		if (isMessageUnavailableError(error)) {
			log$15.debug(`Skip remove typing indicator for unavailable message`, { messageId: state.messageId });
			return;
		}
		log$15.debug(`Failed to remove typing indicator`, {
			messageId: state.messageId,
			error: error instanceof Error ? error.message : error
		});
	}
...
const log$14 = larkLogger("card/cardkit");
/**
...
function logCardKitResponse(params) {
	...
	const { code, msg } = resp;
	log$14.info(`cardkit ${api} response`, {
		code,
		msg,
		context
	});
	if (code && code !== 0) {
		log$14.warn(`cardkit ${api} FAILED`, {
			code,
			msg,
			context,
			fullResponse: resp
...
const log$13 = larkLogger("card/image-resolver");
/** Matches complete markdown image syntax: `![alt](value)` */
const IMAGE_RE = /!\[([^\]]*)\]\(([^)\s]+)\)/g;
	...
	*/
	async resolveImagesAwait(text, timeoutMs) {
		...
		if (this.pending.size > 0) {
			log$13.info("resolveImagesAwait: waiting for uploads", {
			...
			if (this.pending.size > 0) log$13.warn("resolveImagesAwait: timed out with pending uploads", { remaining: this.pending.size });
		}
	...
	async doUpload(url) {
		try {
			log$13.info("uploading image", { url });
			...
			log$13.warn("image upload failed", {
				url,
				error: String(err)
			});
			this.pending.delete(url);
...
*/
const log$12 = larkLogger("card/unavailable-guard");
var UnavailableGuard = class {
	...
	*/
	terminate(source, err) {
		...
		log$12.warn("reply pipeline terminated by unavailable message", {
			source,
			apiCode,
...
*/
const log$11 = larkLogger("card/streaming");
...
var StreamingCardController = class {
	...
	*/
	...
	transition(to, source, reason) {
		...
		if (!PHASE_TRANSITIONS[from].has(to)) {
			log$11.warn("phase transition rejected", {
				from,
				to,
				source
			});
	...
	*/
	...
	async onPartialReply(payload) {
		...
		log$11.debug("onPartialReply", { len: text.length });
		if (!text) return;
		if (!this.reasoning.reasoningStartTime) this.reasoning.reasoningStartTime = Date.now();
		...
		if (!this.text.streamingPrefix && SILENT_REPLY_TOKEN.startsWith(this.text.accumulatedText.trim())) {
			log$11.debug("onPartialReply: buffering NO_REPLY prefix");
			return;
		}
		await this.ensureCardCreated();
	...
	async onError(err, info) {
		...
		log$11.error(`${info.kind} reply failed`, { error: String(err) });
	...
	async onIdle() {
		...
		if (this.cardKit.cardMessageId) try {
			if (idleEffectiveCardId) {
				...
				log$11.info("onIdle: closing streaming mode", {
			...
			if (!this.text.completedText && !this.text.accumulatedText) log$11.warn("reply completed without visible text, using empty-reply fallback");
			...
			if (idleEffectiveCardId) {
				...
				this.cardKit.cardKitSequence += 1;
				log$11.info("onIdle: updating final card", {
			...
			log$11.info("reply completed, card finalized", {
				elapsedMs: this.elapsed(),
				isCardKit: !!idleEffectiveCardId
			});
		} catch (err) {
			log$11.warn("final card update failed", { error: String(err) });
		}
	}
	markFullyComplete() {
		log$11.debug("markFullyComplete", {
	...
	async abortCard() {
		try {
			...
			if (effectiveCardId) {
				...
				log$11.info("abortCard completed", { effectiveCardId });
				...
				log$11.info("abortCard completed (IM fallback)", { messageId: this.cardKit.cardMessageId });
			}
		} catch (err) {
			log$11.warn("abortCard failed", { error: String(err) });
		}
	}
			...
			try {
				try {
					...
					if (this.isStaleCreate(epoch)) {
						log$11.info("ensureCardCreated: stale epoch after createCardEntity, bailing out", {
							epoch,
							phase: this.phase
						});
						return;
					...
					if (cId) {
						...
						log$11.info("created CardKit entity", {
							cardId: cId,
						...
						if (this.isStaleCreate(epoch)) {
							log$11.info("ensureCardCreated: stale epoch after sendCardByCardId, bailing out", {
								epoch,
								phase: this.phase
							});
							this.disposeShutdownHook?.();
						...
						log$11.info("sent CardKit card", { messageId: result.messageId });
					...
					log$11.warn("CardKit flow failed, falling back to IM", { apiDetail });
					...
					if (this.isStaleCreate(epoch)) {
						log$11.info("ensureCardCreated: stale epoch after IM fallback send, bailing out", {
							epoch,
							phase: this.phase
						});
						return;
					...
					log$11.info("sent fallback IM card", { messageId: result.messageId });
				}
			} catch (err) {
				if (this.isStaleCreate(epoch)) return;
				if (this.guard.terminate("ensureCardCreated.outer", err)) return;
				log$11.warn("thinking card failed, falling back to static", { error: String(err) });
	...
	async performFlush() {
		...
		if (!this.cardKit.cardKitCardId && this.cardKit.originalCardKitCardId) {
			log$11.debug("performFlush: skipping (CardKit streaming disabled, awaiting final update)");
			return;
		}
		log$11.debug("flushCardUpdate: enter", {
		...
		try {
			...
			if (this.cardKit.cardKitCardId) {
				...
				this.cardKit.cardKitSequence += 1;
				log$11.debug("flushCardUpdate: seq bump", {
					seqBefore: prevSeq,
					seqAfter: this.cardKit.cardKitSequence
				});
				...
				log$11.debug("flushCardUpdate: IM patch fallback");
			...
			if (apiCode === 230020) {
				log$11.info("flushCardUpdate: rate limited (230020), skipping", { seq: this.cardKit.cardKitSequence });
				return;
			}
			const apiDetail = extractApiDetail(err);
			log$11.error("card stream update failed", {
				apiCode,
				seq: this.cardKit.cardKitSequence,
				apiDetail
			});
			if (this.cardKit.cardKitCardId) {
				log$11.warn("disabling CardKit streaming, falling back to im.message.patch");
				this.cardKit.cardKitCardId = null;
			}
		}
	}
	...
	*/
	async closeStreamingAndUpdate(cardId, card, label) {
		const seqBeforeClose = this.cardKit.cardKitSequence;
		this.cardKit.cardKitSequence += 1;
		log$11.info(`${label}: closing streaming mode`, {
			seqBefore: seqBeforeClose,
			seqAfter: this.cardKit.cardKitSequence
		});
		await setCardStreamingMode({
		...
		const seqBeforeUpdate = this.cardKit.cardKitSequence;
		this.cardKit.cardKitSequence += 1;
		log$11.info(`${label}: updating card`, {
			seqBefore: seqBeforeUpdate,
			seqAfter: this.cardKit.cardKitSequence
		});
		await updateCardKitCard({
...
*/
const log$10 = larkLogger("card/reply-dispatcher");
function createFeishuReplyDispatcher(params) {
	...
	log$10.info("reply mode resolved", {
		effectiveReplyMode,
		replyMode,
		chatType
	});
	...
	const typingCallbacks = createTypingCallbacks({
		...
		start: async () => {
			...
			if (typingStopped && typingState) {
				...
				typingState = null;
				log$10.info("removed typing indicator (raced with stop)");
				return;
			}
			log$10.info("added typing indicator reaction");
		},
		stop: async () => {
			typingStopped = true;
			...
			typingState = null;
			log$10.info("removed typing indicator reaction");
		},
		onStartError: (err) => {
			logTypingFailure({
				log: (message) => log$10.warn(message),
				channel: "feishu",
				action: "start",
				error: err
			});
	...
	const { dispatcher, replyOptions, markDispatchIdle } = core.channel.reply.createReplyDispatcherWithTyping({
		...
		deliver: async (payload) => {
			log$10.debug("deliver called", { textPreview: payload.text?.slice(0, 100) });
			if (shouldSkip("deliver.entry")) return;
			if (staticAborted || controller?.isTerminated || controller?.isAborted) {
				log$10.debug("deliver: skipped (aborted)");
				return;
			}
			if (dispatchFullyComplete) {
				log$10.debug("deliver: skipped (dispatch already complete)");
				return;
			}
			const text = payload.text ?? "";
			...
			if (!text.trim() && payloadMediaUrls.length === 0) {
				log$10.debug("deliver: empty text and no media, skipping");
				return;
			}
			if (controller) {
				...
				log$10.warn("deliver: card creation failed, falling back to static delivery");
			}
			if (text.trim()) if (shouldUseCard(text)) {
				const chunks = core.channel.text.chunkTextWithMode(text, textChunkLimit, chunkMode);
				log$10.info("deliver: sending card chunks", {
					count: chunks.length,
					chatId
				});
				...
				log$10.info("deliver: sending text chunks", {
					count: chunks.length,
					chatId
				});
				for (const chunk of chunks) try {
			...
			for (const mediaUrl of payloadMediaUrls) {
				...
				try {
					log$10.info("deliver: sending media via static path", { mediaUrl: mediaUrl.slice(0, 80) });
					await sendMediaLark({
						cfg,
						to: chatId,
						mediaUrl,
					...
					log$10.error("deliver: static media send failed", { error: String(mediaErr) });
				}
			}
		},
...
const log$9 = larkLogger("inbound/dispatch-context");
/**
...
function ensureRuntime(runtime) {
	...
	return {
		log: (...args) => log$9.info(args.map(String).join(" ")),
		error: (...args) => log$9.error(args.map(String).join(" ")),
...
async function resolveThreadSessionKey(params) {
	...
	if (!await isThreadCapableGroup({
		...
		log$9.info(`thread session skipped: group ${chatId} is not topic/thread mode`);
		return;
	}
...
const log$8 = larkLogger("inbound/dispatch-commands");
/**
...
async function dispatchSystemCommand(dc, ctxPayload, suppressReply = false, replyToMessageId) {
	...
	log$8.info(`system command detected, plain-text dispatch${suppressReply ? ", reply suppressed" : ""}`);
	...
	log$8.info(`system command dispatched (delivered=${delivered}, elapsed=${ticketElapsed()}ms)`);
}
//#endregion
...
*/
const log$7 = larkLogger("core/token-store");
...
async function getMasterKey() {
	try {
		...
		log$7.warn("master key has unexpected length, regenerating");
	} catch (err) {
		if (!(err instanceof Error) || err.code !== "ENOENT") log$7.warn(`failed to read master key: ${err instanceof Error ? err.message : err}`);
	}
	await ensureLinuxCredDir();
	const key = randomBytes(MASTER_KEY_BYTES);
	await writeFile(MASTER_KEY_PATH, key, { mode: 384 });
	await chmod(MASTER_KEY_PATH, 384);
	log$7.info("generated new master key for encrypted file storage");
	return key;
}
...
async function getWin32MasterKey() {
	try {
		...
		if (key.length === MASTER_KEY_BYTES) return key;
		log$7.warn("win32 master key has unexpected length, regenerating");
	} catch (err) {
		if (!(err instanceof Error) || err.code !== "ENOENT") log$7.warn(`failed to read win32 master key: ${err instanceof Error ? err.message : err}`);
	}
	await ensureWin32CredDir();
	const key = randomBytes(MASTER_KEY_BYTES);
	await writeFile(WIN32_MASTER_KEY_PATH, key);
	log$7.info("generated new master key for win32 encrypted file storage");
	return key;
}
const win32Backend = {
	async get(_service, account) {
...
function createBackend() {
	switch (process.platform) {
		...
		default:
			log$7.warn(`unsupported platform "${process.platform}", falling back to macOS backend`);
...
async function setStoredToken(token) {
	...
	log$7.info(`saved UAT for ${token.userOpenId} (at:${maskToken(token.accessToken)})`);
}
/**
...
async function removeStoredToken(appId, userOpenId) {
	...
	log$7.info(`removed UAT for ${userOpenId}`);
}
/**
...
const log$6 = larkLogger("core/device-flow");
/**
...
async function requestDeviceAuthorization(params) {
	...
	log$6.info(`requesting device authorization (scope="${scope}") url=${endpoints.deviceAuthorization} token_url=${endpoints.token}`);
	...
	log$6.info(`response status=${resp.status} body=${text.slice(0, 500)}`);
	let data;
	try {
	...
	log$6.info(`device_code obtained, expires_in=${expiresIn}s (${Math.round(expiresIn / 60)}min), interval=${interval}s`);
	return {
...
async function pollDeviceToken(params) {
	...
	while (Date.now() < deadline && attempts < MAX_POLL_ATTEMPTS) {
		...
		try {
			...
			log$6.warn(`poll network error: ${err}`);
			interval = Math.min(interval + 1, MAX_POLL_INTERVAL);
			continue;
		}
		const error = data.error;
		if (!error && data.access_token) {
			log$6.info("token obtained successfully");
			const refreshToken = data.refresh_token ?? "";
			const expiresIn = data.expires_in ?? 7200;
			let refreshExpiresIn = data.refresh_token_expires_in ?? 604800;
			if (!refreshToken) {
				log$6.warn("no refresh_token in response, token will not be refreshable");
		...
		if (error === "authorization_pending") {
			log$6.debug("authorization_pending, retrying...");
			continue;
		}
		if (error === "slow_down") {
			interval = Math.min(interval + 5, MAX_POLL_INTERVAL);
			log$6.info(`slow_down, interval increased to ${interval}s`);
			continue;
		}
		if (error === "access_denied") {
			log$6.info("user denied authorization");
			return {
				ok: false,
				error: "access_denied",
				message: "用户拒绝了授权"
		...
		if (error === "expired_token" || error === "invalid_grant") {
			log$6.info(`device code expired/invalid (error=${error})`);
			return {
				ok: false,
				error: "expired_token",
		...
		log$6.warn(`unexpected error: error=${error}, desc=${desc}`);
		return {
			ok: false,
			error: "expired_token",
			message: desc
	...
	if (attempts >= MAX_POLL_ATTEMPTS) log$6.warn(`max poll attempts (${MAX_POLL_ATTEMPTS}) reached`);
	return {
		ok: false,
		error: "expired_token",
		message: "授权超时，请重新发起"
...
const log$5 = larkLogger("core/uat-client");
/**
...
async function doRefreshToken(opts, stored) {
	if (Date.now() >= stored.refreshExpiresAt) {
		log$5.info(`refresh_token expired for ${opts.userOpenId}, clearing`);
	...
	if (code !== void 0 && code !== 0 || error) {
		...
		if (REFRESH_TOKEN_RETRYABLE.has(code)) {
			log$5.warn(`refresh transient error (code=${errCode}) for ${opts.userOpenId}, retrying once`);
			...
			if (retryCode !== void 0 && retryCode !== 0 || retryError) {
				...
				log$5.warn(`refresh failed after retry (code=${retryErrCode}), clearing token for ${opts.userOpenId}`);
				await removeStoredToken(opts.appId, opts.userOpenId);
				return null;
			}
		} else {
			log$5.warn(`refresh failed (code=${errCode}), clearing token for ${opts.userOpenId}`);
			await removeStoredToken(opts.appId, opts.userOpenId);
			return null;
		}
	}
	...
	log$5.info(`refreshed UAT for ${opts.userOpenId} (at:${maskToken(updated.accessToken)})`);
	return updated;
}
/**
...
async function callWithUAT(opts, apiCall) {
	...
	try {
		...
		if (TOKEN_RETRY_CODES.has(code)) {
			log$5.warn(`API call failed (code=${code}), refreshing and retrying`);
			const stored = await getStoredToken(opts.appId, opts.userOpenId);
			if (!stored) throw new NeedAuthorizationError(opts.userOpenId);
			const refreshed = await refreshWithLock(opts, stored);
			if (!refreshed) throw new NeedAuthorizationError(opts.userOpenId);
...
async function revokeUAT(appId, userOpenId) {
	...
	log$5.info(`revoked UAT for ${userOpenId}`);
}
//#endregion
...
const log$4 = larkLogger("inbound/dispatch");
/**
...
async function dispatchNormalMessage(dc, ctxPayload, chatHistories, historyKey, historyLimit, replyToMessageId, skillFilter, skipTyping) {
	if (isLikelyAbortText(dc.ctx.content?.trim() ?? "")) {
		...
		log$4.info("abort message detected, using plain-text dispatch");
	...
	log$4.info(`dispatching to agent (session=${effectiveSessionKey})`);
	try {
		...
		log$4.info(`dispatch complete (replies=${counts.final}, elapsed=${ticketElapsed()}ms)`);
	} finally {
...
async function dispatchToAgent(params) {
	...
	if (i18nCommandName) {
		...
		log$4.info(`${i18nCommandName} command detected, using i18n dispatch`);
		try {
			let i18nTexts;
...
const log$3 = larkLogger("tools/auto-auth");
/**
* 防抖缓冲区 Map。
*
* Key 规则：
...
function enqueueAuthRequest(bufferKey, scopes, ctx, flushFn, debounceMs = AUTH_DEBOUNCE_MS) {
	...
	if (existing) {
		...
		if (existing.phase === "executing") {
			log$3.info(`auth in-flight, piggyback → key=${bufferKey}, scopes=[${[...existing.scopes].join(", ")}]`);
			...
			existing.updateTimer = setTimeout(async () => {
				...
				if (existing.isUpdating) {
					...
					log$3.info(`scope update deferred (previous update still running) → key=${bufferKey}`);
					return;
				}
				existing.isUpdating = true;
				try {
					const mergedScopes = [...existing.scopes];
					log$3.info(`scope update flush → key=${bufferKey}, scopes=[${mergedScopes.join(", ")}]`);
					await existing.flushFn(mergedScopes);
				} catch (err) {
					log$3.warn(`scope update failed: ${err}`);
				} finally {
					existing.isUpdating = false;
					if (existing.pendingReupdate) {
						existing.pendingReupdate = false;
						const finalScopes = [...existing.scopes];
						log$3.info(`scope reupdate → key=${bufferKey}, scopes=[${finalScopes.join(", ")}]`);
						try {
							await existing.flushFn(finalScopes);
						} catch (err) {
							log$3.warn(`scope reupdate failed: ${err}`);
						}
					}
				}
		...
		log$3.info(`debounce merge → key=${bufferKey}, scopes=[${[...existing.scopes].join(", ")}]`);
		return new Promise((resolve, reject) => {
	...
	entry.timer = setTimeout(async () => {
		...
		log$3.info(`debounce flush → key=${bufferKey}, waiters=${entry.waiters.length}, scopes=[${mergedScopes.join(", ")}]`);
...
function addToDeferredUserAuth(ticket, scopes, account, cfg) {
	...
	if (existing) {
		for (const s of scopes) existing.scopes.add(s);
		log$3.info(`deferred user auth scope merge → key=${key}, scopes=[${[...existing.scopes].join(", ")}]`);
	} else {
		...
		log$3.info(`deferred user auth created → key=${key}, scopes=[${scopes.join(", ")}]`);
	}
}
/** v2 卡片 i18n 配置 */
const I18N_CONFIG = {
...
async function sendAppScopeCard(params) {
	...
	if (appAuthFlows.getByDedupKey(dedup)) {
		log$3.info(`dedup – app-scope card already pending for chatId=${chatId}, scopes=[${missingScopes.join(", ")}], skipping duplicate send`);
		return json({
	...
	if (activeEntry) {
		...
		if (!migrated) log$3.info(`migrate raced, falling through to new card creation`);
		else try {
			...
			log$3.info(`app-scope card updated in-place, cardId=${activeFlow.cardId}, seq=${newSeq}, scopes=[${missingScopes.join(", ")}]`);
			...
			log$3.warn(`failed to update existing app-scope card, creating new one: ${err}`);
		}
	}
	...
	if (!cardId) {
		log$3.warn("createCardEntity failed for app-scope card, falling back");
		return json({
	...
	log$3.info(`app-scope card sent, operationId=${operationId}, scopes=[${missingScopes.join(", ")}]`);
	return json({
		awaiting_app_authorization: true,
		message: "已向用户发送授权引导卡片，等待用户完成授权操作。请告知用户：按照卡片提示完成授权，完成后系统将自动重试之前的操作。请等待用户完成卡片操作，不要建议其他替代方案。",
		missing_scopes: missingScopes
...
async function handleCardAction(data, cfg, accountId) {
	...
	if (!flow) {
		log$3.warn(`card action ${operationId} not found (expired or already handled)`);
		return;
	}
	log$3.info(`app_auth_done clicked by ${senderOpenId}, operationId=${operationId}`);
	invalidateAppScopeCache(flow.appId);
	const acct = getLarkAccount(flow.cfg, flow.accountId);
	if (!acct.configured) {
		log$3.warn(`account ${flow.accountId} not configured, skipping OAuth`);
		return;
	}
	const sdk = LarkClient.fromAccount(acct).sdk;
	let grantedScopes = [];
	try {
		...
		log$3.warn(`failed to re-check app scopes: ${err}, proceeding anyway`);
	}
	if (!isAppScopeSatisfied(grantedScopes, flow.requiredScopes, flow.scopeNeedType)) {
		log$3.warn(`app scopes still missing after user confirmation: [${flow.requiredScopes.join(", ")}]`);
		return { toast: {
			type: "error",
			content: "权限尚未开通，请确认已申请并审核通过后再试"
		} };
	}
	log$3.info(`app scopes verified, proceeding with OAuth`);
	...
	if (consumedDeferred && deferKey) {
		deferredUserAuth.delete(deferKey);
		log$3.info(`consumed deferred user auth scopes: [${[...consumedDeferred.scopes].join(", ")}]`);
	}
	...
	setImmediate(async () => {
		try {
			try {
				...
				log$3.warn(`failed to update app-scope card to progress via API: ${err}`);
			}
			if (!flow.ticket.senderOpenId) {
				log$3.warn("no senderOpenId in ticket, skipping OAuth");
				return;
			}
			...
			if (userBatch) {
				...
				log$3.info(`merged user batch scopes into app auth completion: [${[...mergedScopes].join(", ")}]`);
			}
			if (mergedScopes.size === 0) {
				log$3.info("no business scopes to authorize after app auth, sending synthetic message for retry");
				...
				await promise;
				log$3.info("synthetic message dispatched after app-auth-only completion");
			...
			log$3.error(`handleCardAction background task failed: ${err}`);
		}
	});
	return {
		toast: {
...
async function handleInvokeErrorWithAutoAuth(err, cfg) {
	...
	if (ticket) {
				...
				try {
					...
					if (acct.configured) {
						if (hasActiveAppAuthForMessage(ticket)) {
							...
							log$3.info(`UserAuthRequiredError deferred (app auth pending), scopes=[${scopes.join(", ")}]`);
							return json({
								awaiting_app_authorization: true,
						...
						log$3.info(`UserAuthRequiredError → enqueue, key=${bufferKey}, scopes=[${scopes.join(", ")}]`);
					...
					log$3.warn(`executeAuthorize failed: ${autoAuthErr}, falling back`);
				}
			}
			if (err instanceof UserScopeInsufficientError) {
				...
				try {
					...
					if (acct.configured) {
						if (hasActiveAppAuthForMessage(ticket)) {
							addToDeferredUserAuth(ticket, scopes, acct, cfg);
							log$3.info(`UserScopeInsufficientError deferred (app auth pending), scopes=[${scopes.join(", ")}]`);
							return json({
								awaiting_app_authorization: true,
								user_auth_deferred: true,
								message: "应用权限尚未开通，将在应用权限通过后自动为您发起用户授权。请先按照应用权限卡片的提示完成操作。请等待用户完成卡片操作，不要建议其他替代方案。",
						...
						const bufferKey = `user:${ticket.accountId}:${senderOpenId}:${ticket.messageId}`;
						log$3.info(`UserScopeInsufficientError → enqueue, key=${bufferKey}, scopes=[${scopes.join(", ")}]`);
						return await enqueueAuthRequest(bufferKey, scopes, {
							account: acct,
							cfg,
							ticket
		...
		} else log$3.error(`senderOpenId not found ${err}`);
			...
			try {
				...
				if (acct.configured) {
					if (senderOpenId && appScopeErr.allRequiredScopes?.length) {
						...
						log$3.info(`AppScopeMissingError → deferred allRequiredScopes=[${appScopeErr.allRequiredScopes.join(", ")}]`);
					}
					const bufferKey = `app:${ticket.accountId}:${ticket.chatId}:${ticket.messageId}`;
					log$3.info(`AppScopeMissingError → enqueue, key=${bufferKey}, scopes=[${appScopeErr.missingScopes.join(", ")}]`);
				...
				log$3.warn(`sendAppScopeCard failed: ${cardErr}, falling back`);
			}
		}
	} else log$3.error(`ticket not found ${err}`);
...
const log$2 = larkLogger("tools/oauth");
...
async function verifyTokenIdentity(brand, accessToken, expectedOpenId) {
	...
	try {
		...
		if (data.code !== 0) {
			log$2.warn(`user_info API error: code=${data.code}, msg=${data.msg}`);
			return { valid: false };
		}
		const actualOpenId = data.data?.open_id;
		if (!actualOpenId) {
			log$2.warn("user_info API returned no open_id");
			return { valid: false };
		}
		return {
		...
		log$2.warn(`identity verification request failed: ${err}`);
		return { valid: false };
	}
}
	...
	registerTool(api, {
		...
		async execute(_toolCallId, params) {
			...
			try {
				...
				log$2.error(`${p.action} failed: ${err}`);
				return json({ error: formatLarkError(err) });
			}
		}
	}, { name: "feishu_oauth" });
...
async function executeAuthorize(params) {
	...
	try {
		...
		if (err instanceof OwnerAccessDeniedError) {
			log$2.warn(`non-owner user ${senderOpenId} attempted to authorize`);
			return json({
				error: "permission_denied",
	...
	if (existing && tokenStatus(existing) !== "expired") if (effectiveScope) {
		...
		if (missingScopes.length > 0) log$2.info(`existing token missing scopes [${missingScopes.join(", ")}], starting incremental auth`);
		else {
			if (onAuthComplete) try {
				await onAuthComplete();
			} catch (e) {
				log$2.warn(`onAuthComplete failed: ${e}`);
			}
			return json({
				success: true,
	...
	if (pendingFlows.has(flowKey)) {
		...
		if (oldFlow.messageId === currentMessageId) {
			...
			if (oldFlow.scope) {
				...
				log$2.info(`scope merge on reuse: [${[...merged].join(", ")}]`);
			}
			log$2.info(`same message, replacing flow for user=${senderOpenId}, app=${appId}, reusing cardId=${reuseCardId}`);
		} else {
			oldFlow.superseded = true;
			oldFlow.controller.abort();
			pendingFlows.delete(flowKey);
			log$2.info(`new message, cancelling old flow for user=${senderOpenId}, app=${appId}, old cardId=${oldFlow.cardId}`);
			try {
				await updateCardKitCardForAuth({
					cfg,
				...
				log$2.warn(`failed to update old card to expired: ${e}`);
			}
		}
	}
	...
	if (effectiveScope) try {
		...
		if (unavailableScopes.length > 0) {
			log$2.info(`app has not granted scopes [${unavailableScopes.join(", ")}], filtering them out`);
			...
			log$2.info(`proceeding with available scopes [${availableScopes.join(", ")}]`);
		}
	} catch (err) {
		log$2.warn(`failed to check app scopes, proceeding anyway: ${err}`);
	}
	...
	if (reuseCardId) {
		...
		try {
			...
			log$2.info(`updated existing card ${reuseCardId} with merged scopes, seq=${newSeq}`);
		} catch (err) {
			log$2.warn(`failed to update existing card, creating new one: ${err}`);
	...
	pollDeviceToken({
		...
		if (currentFlow.superseded) {
			log$2.info(`flow superseded, skipping card update for cardId=${cardId}`);
			return;
		}
		if (result.ok) {
			const identity = await verifyTokenIdentity(brand, result.token.accessToken, senderOpenId);
			if (!identity.valid) {
				log$2.warn(`identity mismatch! expected=${senderOpenId}, actual=${identity.actualOpenId ?? "unknown"}, cardId=${cardId}`);
				try {
					await updateCardKitCardForAuth({
						cfg,
						cardId,
					...
					log$2.warn(`failed to update card for identity mismatch: ${e}`);
				}
				pendingFlows.delete(flowKey);
			...
			try {
				...
				log$2.warn(`failed to update card to success: ${e}`);
			}
			pendingFlows.delete(flowKey);
			pendingFlowDelete = true;
			if (onAuthComplete) try {
			...
			if (skipSyntheticMessage) log$2.info("skipSyntheticMessage=true, skipping synthetic message");
			else try {
				...
				log$2.info(`synthetic message queued (${status})`);
				await promise;
				log$2.info("synthetic message dispatched after successful auth");
			} catch (e) {
				log$2.warn(`failed to send synthetic message after auth: ${e}`);
			}
		} else {
			try {
				await updateCardKitCardForAuth({
				...
				log$2.warn(`failed to update card to failure: ${e}`);
			}
			pendingFlows.delete(flowKey);
			pendingFlowDelete = true;
		}
	}).catch((err) => {
		log$2.error(`polling error: ${err}`);
	}).finally(() => {
...
const log$1 = larkLogger("tools/onboarding-auth");
...
async function triggerOnboarding(params) {
	...
	if (!acct.configured) {
		log$1.warn(`account ${accountId} not configured, skipping`);
		return;
	}
	const sdk = LarkClient.fromAccount(acct).sdk;
	const { appId } = acct;
	...
	if (!ownerOpenId) {
		log$1.info(`app ${appId} has no owner info, skipping`);
		return;
	}
	if (userOpenId !== ownerOpenId) {
		log$1.info(`user ${userOpenId} is not app owner (${ownerOpenId}), skipping`);
		return;
	}
	log$1.info(`user ${userOpenId} is app owner, starting OAuth`);
	let allUserScopes;
	try {
		allUserScopes = await getAppGrantedScopes(sdk, appId, "user");
	} catch (err) {
		log$1.warn(`failed to get app granted scopes: ${err}`);
		return;
	}
	allUserScopes = filterSensitiveScopes(allUserScopes);
	if (allUserScopes.length === 0) {
		log$1.info("no user scopes configured, skipping");
		return;
	}
	const batches = [];
	for (let i = 0; i < allUserScopes.length; i += MAX_SCOPES_PER_BATCH) batches.push(allUserScopes.slice(i, i + MAX_SCOPES_PER_BATCH));
	log$1.info(`${allUserScopes.length} user scopes, ${batches.length} batch(es)`);
	const startBatch = async (batchIndex) => {
		if (batchIndex >= batches.length) {
			log$1.info("all batches completed");
			return;
		}
		...
		log$1.info(`starting batch ${batchIndex + 1}/${batches.length}, scopes=${batch.length}`);
		try {
			await executeAuthorize({
				...
				onAuthComplete: async () => {
					log$1.info(`batch ${batchIndex + 1}/${batches.length} auth completed`);
					await startBatch(batchIndex + 1);
				}
			});
		} catch (err) {
			log$1.error(`batch ${batchIndex + 1} failed: ${err}`);
		}
	};
	await startBatch(0);
}
...
function handleAskUserAction(data, _cfg, accountId) {
	...
	try {
		...
		log.info(`card action received: tag=${actionTag}, name=${actionName}, chat=${openChatId}, sender=${senderOpenId}, hasFormValue=${!!formValue}, hasValue=${!!event.action?.value}`);
	...
	if (!ctx && openChatId) {
		...
		if (ctx) log.info(`resolved question via chat-scoped fallback: ${ctx.questionId}`);
	}
	if (!ctx) {
		if (operationId) log.warn(`ask-user action: question ${operationId} not found (expired or already handled)`);
...
//#endregion
export { mentionedBot as $, openPlatformDomain as A, extractMentionOpenId as B, analyzeTrace as C, traceByMessageId as D, runDiagnosis as E, checkMessageGate as F, updateCardFeishu as G, editMessageFeishu as H, getMessageFeishu as I, extractMessageBody as J, buildMentionedCardContent as K, parseMessageEvent as L, probeFeishu as M, filterSensitiveScopes as N, resolveAnyEnabledToolsConfig as O, getStoredToken as P, formatMentionForText as Q, buildConvertContextFromItem as R, registerCommands as S, formatTraceOutput as T, sendCardFeishu as U, getAppGrantedScopes as V, sendMessageFeishu as W, formatMentionAllForText as X, formatMentionAllForCard as Y, formatMentionForCard as Z, handleInvokeErrorWithAutoAuth as _, resolveReceiveIdType as _t, registerAskUserQuestionTool as a, assertLarkOk as at, getFirstAccount as b, registerFeishuOAuthTool as c, sendFileLark as ct, isInvokeError as d, uploadFileLark as dt, nonBotMentions as et, json as f, uploadImageLark as ft, unixTimestampToISO8601 as g, parseFeishuRouteTarget as gt, parseTimeToTimestampMs as h, normalizeFeishuTarget as ht, isMessageExpired as i, sendTextLark as it, wwwDomain as j, mcpDomain as k, StringEnum as l, sendImageLark as lt, parseTimeToTimestamp as m, looksLikeFeishuId as mt, monitor_exports as n, sendCardLark as nt, triggerOnboarding as o, formatLarkError as ot, parseTimeToRFC3339 as p, validateLocalMediaRoots as pt, buildMentionedMessage as q, handleFeishuReaction as r, sendMediaLark as rt, executeAuthorize as s, sendAudioLark as st, monitorFeishuProvider as t, resolveFeishuGroupToolPolicy as tt, convertTimeRange as u, uploadAndSendMediaLark as ut, createToolContext as v, formatDiagReportCli as w, registerTool as x, formatToolResult as y, convertMessageContent as z };
